...
MODULE_STREAMSB_REGEXP_URL='https://\(www\.\)\?\(streamsb\.\(com\|net\)\)/'

MODULE_STREAMSB_UPLOAD_OPTIONS="
...
MODULE_STREAMSB_UPLOAD_REMOTE_SUPPORT=no

streamsb_upload(){
  local -r FILE=$2
  local -r API_URL='https://api.streamsb.com/api'
  ...
  # Start upload
   ...
   CODE=$(parse_json 'code' <<<"$UPLOADEDFILE")
  ...
  if [[ $STATUS != "OK" ]]; then
    ...
    echo "https://onscreensvideo.com/e/$CODE.html"
  fi
