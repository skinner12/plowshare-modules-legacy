...
MODULE_FILEMOON_REGEXP_URL='https://\(www\.\)\?\(filemoon\.\(com\|net|sx\)\)/'

MODULE_FILEMOON_UPLOAD_OPTIONS="
...
MODULE_FILEMOON_UPLOAD_REMOTE_SUPPORT=no

filemoon_upload(){
    local -r FILE=$2
    local -r API_URL='https://filemoon.sx/api'
    ...
    CODE=$(parse_json 'filecode' <<<"$UPLOADEDFILE")
    FILENAME=$(parse_json 'filename' <<<"$UPLOADEDFILE")
    ...
    if [[ $STATUS != "200" ]]; then
        ...
        echo "https://filemoon.wf/e/$CODE/$FILENAME"
    fi
